...
### Fixed
- Use FastMCP's public `fastmcp.tools` import for `ToolResult` and constrain FastMCP to compatible 4.x releases to prevent dependency drift from breaking server startup.
...
### Fixed
- Quote database identifiers inside `refresh_catalog` async `INFORMATION_SCHEMA` scan queries. This completes support for Snowflake-managed share databases and per-user personal databases whose names contain characters such as `$`, `@`, or `.`. (#49)
...
### Fixed
- Reissue the 0.2.0 release as 0.2.1 so the GitHub release includes the MCPB desktop extension asset.
