...
def test_checkpoint_functionality():
    ...
    assert len(loaded_results) == 3, f"Should load 3 total results, got {len(loaded_results)}"
    assert len(processed_databases) == 2, f"Should have 2 databases, got {len(processed_databases)}"
    print(f"✓ Loaded {len(loaded_results)} results from {len(processed_databases)} databases")

    # Test 4: Error log
    print("\n--- Test 4: Error Log ---")

    test_errors = {
        'FAILED_DB': 'Connection timeout',
        'RESTRICTED_DB': 'Access denied'
    }
    ...
    assert loaded_errors['FAILED_DB'] == 'Connection timeout', "Error message should match"
    ...
    assert len(cache.databases) == 2, f"Should have 2 databases, got {len(cache.databases)}"
    ...
    assert len(checkpoint_files) == 0, f"Should have 0 checkpoint files, got {len(checkpoint_files)}"
